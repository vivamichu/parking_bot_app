...
def init_db(db_path: Optional[str] = None, reset: bool = False) -> None:
    ...
    try:
        cur = conn.cursor()
        if reset:
            for t in ("parking_spots", "pricing", "working_hours", "reservations"):
...
def insert_reservation(
    ...
    try:
        cur = conn.execute(
            ...
            "(first_name, last_name, car_plate, start_time, end_time, space_type, status) "
            "VALUES (?,?,?,?,?,?, 'pending')",
            (first_name, last_name, car_plate, start_time, end_time, space_type),
        )
        conn.commit()
