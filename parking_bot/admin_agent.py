...
def apply_decision(reservation_id: int, decision: str, reason: str) -> str:
    ...
    # Stage 3: on a genuine approval, persist the reservation to the MCP-backed
    # reservation store. A recording failure must not abort the confirmation.
    record_note = ""
    if status == "confirmed" and not already_confirmed:
        try:
            recording.record_confirmed(res)
        except Exception as exc:  # noqa: BLE001
            record_note = " (warning: could not record to reservation store)"
            import logging

            logging.getLogger(__name__).warning(
                "Failed to record confirmed reservation %s: %s", reservation_id, exc
            )
    ...
    return (
        f"Decision applied to reservation {reservation_id}: {status} ({reason})"
        + record_note
    )
