...
def test_create_pending_reservation_persists_session_id(temp_db):
    """Stage 2: the visitor's session id must reach the stored row so the admin
    agent can later notify the right chat session."""
    from parking_bot import dynamic_db

    result = reservation.create_pending_reservation(
        ReservationDetails(
            "Aya", "Rai", "342GHB01",
            "2026-07-03 09:00", "2026-07-20 18:00", "standard",
        ),
        session_id="sess-42",
        db_path=temp_db,
    )
    assert result["ok"] is True
    row = dynamic_db.get_reservation(result["reservation_id"], db_path=temp_db)
