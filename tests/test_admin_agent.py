...
def test_apply_decision_approve_confirms_notifies_and_records(default_db, store_path):
    rid = _seed_pending(default_db, session_id="sess-approve")
    ...
    notes = dynamic_db.get_unseen_notifications("sess-approve", db_path=default_db)
    assert len(notes) == 1
...
def test_apply_decision_reject_cancels_notifies_and_records_nothing(default_db, store_path):
    rid = _seed_pending(default_db, session_id="sess-reject")
    out = admin_agent.apply_decision.invoke(
    ...
    notes = dynamic_db.get_unseen_notifications("sess-reject", db_path=default_db)
    assert notes and "REFUSED" in notes[0]["message"]

    # A rejection must NOT write to the confirmed-reservation store.
    assert reservation_store.read_entries(store_path) == []
